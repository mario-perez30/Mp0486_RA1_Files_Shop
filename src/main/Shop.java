			...
			// De aquí en adelante se podría borrar
		        if (this.inventory == null || this.inventory.isEmpty()) {
		            ...
		            while (line != null) {
		                ...
		                String name = null;
		                double wholesaler = 0.0;
		                int stock = 0;

		                for (String sec : sections) {
		                    sec = sec.trim();
		                    if (sec.startsWith("Product:")) {
		                        name = sec.substring(sec.indexOf(":") + 1);
		                    } else if (sec.toLowerCase().startsWith("wholesaler price:")) {
		                        String val = sec.substring(sec.indexOf(":") + 1).replace(',', '.');
		                        try {
		                            wholesaler = Double.parseDouble(val);
		                        } catch (NumberFormatException e) {
		                            wholesaler = 0.0;
		                        }
		                    } else if (sec.toLowerCase().startsWith("stock:")) {
		                        String val = sec.substring(sec.indexOf(":") + 1);
		                        try {
		                ...
		                if (name != null) {
		                    this.addProduct(new Product(name, new Amount(wholesaler), true, stock));
		                }
		                line = br.readLine();
		            }
		            br.close();
		        ...
		        //hasta aquí se podría borrar y luego quitar los catch
		        this.numberProducts = this.inventory.size();
